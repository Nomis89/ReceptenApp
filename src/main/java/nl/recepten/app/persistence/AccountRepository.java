...
public interface AccountRepository extends CrudRepository<Account, Long> {
	@Query(value = "SELECT * FROM receptenapp.account WHERE account = ?1 AND pass_word = ?2", nativeQuery=true)
	...
	@Query(value = "SELECT * FROM receptenapp.account WHERE account = ?1 OR email = ?2", nativeQuery=true)
